...
Use the `.skills/utility/video-processor/scripts/video_processor.py` script for all video processing tasks. The script provides a simple CLI with the following commands:
...
```bash
uv run .skills/utility/video-processor/scripts/video_processor.py download "https://youtube.com/watch?v=..." output.mp4

# Download audio only (as MP3)
uv run .skills/utility/video-processor/scripts/video_processor.py download "https://youtube.com/watch?v=..." --audio-only

# Show video info without downloading
uv run .skills/utility/video-processor/scripts/video_processor.py download "https://youtube.com/watch?v=..." --info

# Download with subtitles
uv run .skills/utility/video-processor/scripts/video_processor.py download "https://youtube.com/watch?v=..." output.mp4 --subtitle
```
...
```bash
uv run .skills/utility/video-processor/scripts/video_processor.py extract-audio input.mp4 output.wav
```
...
```bash
uv run .skills/utility/video-processor/scripts/video_processor.py to-mp4 input.avi output.mp4
```
...
```bash
uv run .skills/utility/video-processor/scripts/video_processor.py to-webm input.mp4 output.webm
```
...
uv run .skills/utility/video-processor/scripts/video_processor.py transcribe input.mp4 transcript.txt

# Transcribe audio file directly
uv run .skills/utility/video-processor/scripts/video_processor.py transcribe audio.wav transcript.txt
```
...
uv run .skills/utility/video-processor/scripts/video_processor.py extract-audio lecture.mp4 lecture.wav

# 2. Transcribe to SRT subtitles
uv run .skills/utility/video-processor/scripts/video_processor.py transcribe lecture.mp4 lecture.srt --format srt --model small

# 3. Convert to web format
uv run .skills/utility/video-processor/scripts/video_processor.py to-webm lecture.mp4 lecture.webm
```
...
1. Use the to-mp4 command with default settings:
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py to-mp4 old_video.avi output.mp4
   ```
...
2. Then transcribe using the base model (good balance of speed/accuracy):
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py transcribe lecture.mp4 transcript.txt --model base
   ```
...
1. Convert to WebM for web optimization:
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py to-webm presentation.mp4 presentation.webm
   ```
2. Generate SRT subtitle file:
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py transcribe presentation.mp4 subtitles.srt --format srt --model small
   ```
...
1. Use a larger model with language specified for best accuracy:
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py transcribe interview.mp4 transcript.txt --model medium --language es
   ```
2. Optionally create SRT for review:
   ```bash
   uv run .skills/utility/video-processor/scripts/video_processor.py transcribe interview.mp4 transcript.srt --format srt --model medium --language es
   ```
...
2. For each video file, run the conversion and transcription:
   ...
   uv run .skills/utility/video-processor/scripts/video_processor.py to-webm training_videos/video1.mp4 output/video1.webm
   uv run .skills/utility/video-processor/scripts/video_processor.py transcribe training_videos/video1.mp4 output/video1.txt --model base
